...
public class Player : MonoBehaviour
{
    ...
    private void Attack()
    {
        atkDirect=(CrossPlatformInputManager.GetAxisRaw("Vertical"));
        playerAnimator.SetFloat("AtkDire", atkDirect + 1);
        if (isAttacking)
        {
            if (attackDuration > 0)
            {
    ...
    public void WhenHit()
    {
        runSpeed = 0f; //when attack animation is on change to 0;
    }
